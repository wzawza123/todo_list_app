...
在 Inbox / 任意文件视图里，直接把一条任务拖到另一条任务上，它（连同自己的子树）就会成为对方的**最后一个子任务**。
可放下的目标会高亮；自己、自己的子孙、以及会超过 4 级上限的目标不接受放下。拖动只在同一个文件内进行，
想把子任务提回上一级用 `Shift+Tab`。对应接口是 `POST /api/tasks/{id}/move  {"parent_id": "..."}`。
